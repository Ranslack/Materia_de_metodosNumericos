...
public class biseccion extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        btnResolver.setBounds(10, 210, 74, 22);
        ...
        btnReiniciar.setBounds(100, 210, 75, 22);
    ...
    private void btnResolverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnResolverActionPerformed
        ...
        aprox = comp1 * comp2;
        System.out.println("Multiplicación en x" + comp1);
        System.out.println("Multiplicación en y" + comp2);
        
        if(aprox>0)
        {
            JOptionPane.showMessageDialog(rootPane, "Los datos del intervalo que ingreso no contienen la respuesta correcta", "Aviso", JOptionPane.WARNING_MESSAGE);
        ...
        if(aprox<0)
        {
            ...
            do
            {
                comp1 = Grado6 * Math.pow(x, 6) + Grado5 * Math.pow(x, 5) + Grado4 * Math.pow(x, 4) + Grado3 * Math.pow(x, 3) + Grado2 * Math.pow(x, 2) + Grado1 * Math.pow(x, 1) + constante;
                comp2 = Grado6 * Math.pow(y, 6) + Grado5 * Math.pow(y, 5) + Grado4 * Math.pow(y, 4) + Grado3 * Math.pow(y, 3) + Grado2 * Math.pow(y, 2) + Grado1 * Math.pow(y, 1) + constante;
                comp3 = Grado6 * Math.pow(pm, 6) + Grado5 * Math.pow(pm, 5) + Grado4 * Math.pow(pm, 4) + Grado3 * Math.pow(pm, 3) + Grado2 * Math.pow(pm, 2) + Grado1 * Math.pow(pm, 1) + constante;
                aprox2 = comp3*comp1; //el pm por x
                aprox3 = comp3*comp2;  //el pm por y
                
                if(aprox2>0){}
                else{
                        //aqui va si es negativo el resultado de PM por x
                        error = ((pm - x) / pm)*100;
                        error = Math.abs(error);
                        System.out.println("N°De iteraciones: " + iteracion);
                        System.out.println("------------------------");
                        System.out.println("Error: " + error);
                        String Fpm = String.format("%."+decimal+"f", pm);
                        System.out.println("Punto medio: " + Fpm);
                        System.out.println("------------------------");
                        model.addRow(new Object[]{iteracion,error, Fpm});
                        y = pm;
                        pm = (x+pm) / 2;
                ...
                if(iteracion == 100){
                        JOptionPane.showMessageDialog(rootPane, "Se alcanzo el máximo de iteraciones", "Aviso", HEIGHT);
                        salir=true;
                    }
            } while (salir == false);
        }
